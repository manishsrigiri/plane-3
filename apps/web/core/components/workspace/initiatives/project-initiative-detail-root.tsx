...
const EpicRow = memo(
  observer(({ epicLink, workspaceSlug, onRemove, canEdit }: EpicRowProps) => {
        ...
        <div className="flex items-center gap-3 px-4 py-3 hover:bg-custom-background-90">
          ...
          {detail.sub_issues_count > 0 && (
            <div className="w-24 flex-shrink-0">
              <ProgressBar
                completed={detail.completed_sub_issues_count ?? 0}
                total={detail.sub_issues_count}
                size="sm"
              />
            </div>
          )}
...
export const ProjectInitiativeDetailRoot = observer(({ workspaceSlug, projectId, initiativeId }: Props) => {
  const router = useRouter();
  ...
  useEffect(() => {
    if (!workspaceSlug || !projectId || !initiativeId) return;
    if (!initiative) {
      setInitiativeLoading(true);
      initiativeStore
        .fetchProjectInitiativeById(workspaceSlug, projectId, initiativeId)
        .finally(() => setInitiativeLoading(false));
    } else {
      setDescriptionValue(initiative.description_html ?? "<p></p>");
    }
    initiativeStore
      .fetchProjectInitiativeEpics(workspaceSlug, projectId, initiativeId)
      .then(() => setEpicsLoaded(true));
  }, [workspaceSlug, projectId, initiativeId]);
  ...
  useEffect(() => {
    if (!workspaceSlug || !projectId || !initiativeId) return;
    const refresh = () => initiativeStore.refreshProjectProgress(workspaceSlug, projectId, initiativeId);
  ...
  const handleRemoveEpic = useCallback(
    async (epicId: string) => {
      await initiativeStore.removeEpicFromProjectInitiative(workspaceSlug, projectId, initiativeId, epicId);
    },
    [initiativeStore, workspaceSlug, projectId, initiativeId]
  );

  const handleDescriptionChange = useCallback(
    (_: object, html: string) => {
      setDescriptionValue(html);
      initiativeStore.updateProjectInitiative(workspaceSlug, projectId, initiativeId, { description_html: html });
    },
    [initiativeStore, workspaceSlug, projectId, initiativeId]
  );
  ...
  const epicSearchCallback = useCallback(
    (params: TProjectIssuesSearchParams) =>
      initiativeService.searchProjectEpics(workspaceSlug, projectId, params.search ?? ""),
  ...
  const handleAddEpics = useCallback(
    async (selected: ISearchIssueResponse[]) => {
      ...
      await initiativeStore.addEpicsToProjectInitiative(workspaceSlug, projectId, initiativeId, ids);
    },
    [initiativeStore, workspaceSlug, projectId, initiativeId]
  );
  ...
  if (initiativeLoading) {
    return (
      <div className="flex items-center justify-center h-64 text-sm text-custom-text-300">
        Loading initiative...
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-2 text-sm text-custom-text-300">
        ...
        <button
          ...
          onClick={() => router.push(`/${workspaceSlug}/projects/${projectId}/initiatives/`)}
        >
          Back to Initiatives
        </button>
      </div>
    ...
    <div className="h-full w-full flex flex-col">
      ...
      <div className="px-6 py-4 border-b border-custom-border-200">
        ...
        {initiative.description && (
          <p className="mt-1.5 text-sm text-custom-text-300 ml-6">{initiative.description}</p>
        )}
        ...
        <div className="mt-3 flex items-center gap-4 ml-6 text-xs text-custom-text-400">
          ...
          <span className="flex items-center gap-1">
            <Layers className="h-3 w-3" />
            {initiative.completed_epic_count ?? 0}/{initiative.epic_count} epic
            {initiative.epic_count !== 1 ? "s" : ""} done
          </span>
        </div>
      ...
      {workspaceId && descriptionValue !== undefined && (
        <div className="px-6 py-4 border-b border-custom-border-200">
          ...
          <RichTextEditor
            editable
            id={`project-initiative-desc-${initiativeId}`}
        ...
        {!epicsLoaded ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-2 text-center">
            ...
            <p className="text-xs text-custom-text-300">
              Click &quot;Add Epic&quot; to link an existing epic from this project.
            </p>
          </div>
        ) : (
          <div className="group">
